...
func (z *DeleteArgs) UnmarshalMsg(bts []byte) (o []byte, err error) {
	...
	for zb0001 > 0 {
		...
		switch msgp.UnsafeString(field) {
		case "BaseArgs":
			bts, err = z.BaseArgs.UnmarshalMsg(bts)
			if err != nil {
				err = msgp.WrapError(err, "BaseArgs")
				return
			}
...
func (z *DeleteArgs) Msgsize() (s int) {
	s = 1 + 9 + z.BaseArgs.Msgsize() + 4 + msgp.StringPrefixSize + len(z.Key)
	return
}
...
func (z *EraseShardArgs) Msgsize() (s int) {
	s = 1 + 9 + z.BaseArgs.Msgsize() + 7 + msgp.ArrayHeaderSize + (len(z.Shards) * (msgp.IntSize))
	return
}
...
func (z *PutAppendArgs) Msgsize() (s int) {
	s = 1 + 9 + z.BaseArgs.Msgsize() + 4 + msgp.StringPrefixSize + len(z.Key) + 6 + msgp.BytesPrefixSize + len(z.Value) + 3 + msgp.StringPrefixSize + len(z.Op)
	return
}
