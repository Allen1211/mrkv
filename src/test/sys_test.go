...
import (
	...
	"mrkv/src/master"
	etc_master "mrkv/src/master/etc"
	"mrkv/src/netw"
	"mrkv/src/node"
	etc_node "mrkv/src/node/etc"
	"mrkv/src/raft"
	"mrkv/src/replica"
)
...
func clearAllData()  {
	utils.DeleteDir("E:\\MyWorkPlace\\Project\\MultiRaftKV\\data")
	utils.DeleteDir("E:\\MyWorkPlace\\Project\\MultiRaftKV\\logs")
}

func startMaster(id int) *master.ShardMaster {
	addrs := masterAddrs
...
func startMasters(n int) []*master.ShardMaster {
	addrs := masterAddrs
	masterConfs := make([]etc_master.MasterConf, n)
	...
	for i, conf := range masterConfs {
		wg.Add(1)
		go func(j int, config etc_master.MasterConf) {
			server := master.StartServer(config)
			servers[j] = server
			wg.Done()
			if server != nil {
				if err := server.StartRPCServer(); err != nil {
...
func stopMaster(masters ...*master.ShardMaster)  {
	for _, server := range masters {
		server.Kill()
	}
}

func stopNode(nodes ...*node.Node)  {
...
func TestShowNode(t *testing.T) {
	...
	for _, n := range showNodes {
		if n.Found && n.Status != master.NodeNormal.String() {
			t.FailNow()
		}
	}
	...
	n := showNodes[0]
	if !n.Found || n.Id != 1 || n.Status != master.NodeDisconnect.String() {
...
func TestShowGroupShowShards(t *testing.T) {
	...
	for _, g := range groups {
		...
		if g.ShardCnt != master.NShards / 2 {
	...
	if len(shards) != master.NShards {
		t.Fatalf("len of shrads not eq to 32")
		t.FailNow()
	}
	for _, shard := range shards {
		if shard.Status != master.SERVING || (shard.Gid != 100 && shard.Gid != 200) {
			t.Fatalf("%v", shard)
			t.FailNow()
		}
	}
...
func TestCURD(t *testing.T) {
	...
	for i := 0; i < 1000; i++ {
		key := fmt.Sprintf("%d", i)
		reply := cli.Get(key)
		if reply.Err != common.OK || string(reply.Value) != mp[key]{
			t.FailNow()
		}
	}
	for i := 0; i < 1000; i++ {
	...
	for i := 0; i < 1000; i++ {
		...
		if ok && (reply.Err != common.OK || string(reply.Value) != val) || !ok && reply.Err == common.OK {
			t.FailNow()
		}
	}
